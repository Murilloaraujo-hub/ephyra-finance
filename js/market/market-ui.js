/** Ephyra Finance - componentes visuais da central Mercado. */
const MarketUI=(()=>{
  'use strict';
  ...
  function cardCurrency(asset,localCurrency,favorites,cache){
    ...
    return`<article class="card market-card" role="button" tabindex="0" aria-label="Ver detalhes de ${esc(asset.name)}" onclick="Market.showDetail('${asset.code}','fiat')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();this.click()}">
      <div class="flx jb aic gs">${assetHeader({name:asset.name,symbol:asset.code,flag:asset.flag})}<button class="market-card-fav ${fav?'active':''}" aria-label="${fav?'Remover':'Adicionar'} ${asset.code} dos favoritos" onclick="event.stopPropagation();Market.toggleFav('${asset.code}','fiat')"><i class="fas fa-star"></i></button></div>
  ...
  function cardCrypto(asset,localCurrency,favorites,cache){
    ...
    return`<article class="card market-card" role="button" tabindex="0" aria-label="Ver detalhes de ${esc(asset.name)}" onclick="Market.showDetail('${asset.ticker}','crypto')" onkeydown="if(event.key==='Enter'||event.key===' '){event.preventDefault();this.click()}">
      <div class="flx jb aic gs">${assetHeader({name:asset.name,symbol:asset.ticker,icon:asset.icon,image:asset.image})}<button class="market-card-fav ${fav?'active':''}" aria-label="${fav?'Remover':'Adicionar'} ${asset.ticker} dos favoritos" onclick="event.stopPropagation();Market.toggleFav('${asset.ticker}','crypto')"><i class="fas fa-star"></i></button></div>
  ...
  function converterTab(cache,localCurrency){
    ...
    return`<div class="card"><div class="market-section-heading"><div><h3><i class="fas fa-exchange-alt"></i> Conversor universal</h3><p>Moedas e criptomoedas com atualização automática</p></div></div><div class="converter-grid"><div class="fg"><label class="fl">De</label><div class="converter-fields"><input id="conv-from-amount" type="number" value="100" min="0" class="form-input" oninput="Market.convertLive()"><select id="conv-from" class="form-input" onchange="Market.convertLive()">${options}</select></div></div><button class="converter-swap" onclick="Market.swapConverter()" aria-label="Inverter moedas"><i class="fas fa-exchange-alt"></i></button><div class="fg"><label class="fl">Para</label><div class="converter-fields"><input id="conv-to-amount" type="number" min="0" class="form-input" oninput="Market.convertReverse()"><select id="conv-to" class="form-input" onchange="Market.convertLive()">${options}</select></div></div></div><div class="tc2" style="margin-top:1rem"><span id="conv-rate-info" class="txs tm">Aguardando cotação</span></div><div id="conv-result" class="converter-result" style="margin-top:1rem;text-align:center;font-weight:850"></div></div>`;
  }
