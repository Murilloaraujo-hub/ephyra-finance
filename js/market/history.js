const MarketHistory = (() => {
  function generate(assetId, type, cache, points = 30) {
    ...
    const values=MarketAPI.makeSparkline(price,0,Math.max(2,Number(points)||30));
    return values.map((value,index)=>({x:Date.now()-(values.length-index-1)*86400000,y:value}));
  }
