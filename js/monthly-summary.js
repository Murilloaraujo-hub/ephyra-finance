/**
 * Ephyra Finance - Resumo Financeiro Mensal
 * Cria snapshots imutáveis dos meses encerrados e os exibe no próprio drawer.
 */
const MonthlySummary = (() => {
  ...
  function money(value) {
    const currency = data?.market?.defaultCurrency || data?.config?.moeda || 'BRL';
    try {
  ...
  function renderPage() {
    ...
    container.innerHTML = `<div class="financial-summary-page">
      <header class="financial-summary-header">
        <div><h1>Resumo Financeiro</h1><p>Acompanhe sua evolução financeira mês a mês e descubra como seus hábitos mudaram ao longo do tempo.</p></div>
  ...
  function renderChart(canvas, summary) {
    ...
    const chart = new Chart(canvas, {
      ...
      options: { responsive: true, maintainAspectRatio: false, cutout: '67%', animation: { duration: 900 }, plugins: { legend: { position: 'bottom', labels: { color: '#94a3b8', usePointStyle: true, padding: 14, font: { size: 11 } } } } }
    });
  ...
  return { init, renderPage, scrollToMonth, generate, ensureCompletedMonths };
})();
