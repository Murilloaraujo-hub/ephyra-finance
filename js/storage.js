const EphyraStorage = (() => {
  ...
  async function getSession(){await init();const temporary=getTemporarySession();if(temporary)return{...temporary,id:'current',remember:false};if(_sessionCache)return{..._sessionCache};const row=await EphyraDB.get('sessions','current');_sessionCache=row||null;return _sessionCache?{..._sessionCache}:null;}
  async function saveSession(sess){
    await init();
    const payload={id:'current',email:_k(sess?.email),remember:Boolean(sess?.remember)};
    if(!payload.email)throw new Error('Sessão inválida');
    try{sessionStorage.removeItem(TEMP_SESSION)}catch{}
    if(payload.remember){
      ...
      sessionStorage.setItem(TEMP_SESSION,JSON.stringify({email:payload.email,remember:false}));
    }
  }
  ...
  function sanitize(d) {
    ...
    const cleanTransaction=item=>{
      const cleaned={...item,valor:cleanMoney(item?.valor),nome:String(item?.nome||'Movimentação'),tipo:item?.tipo==='receita'?'receita':'despesa'};
    ...
    d.config.baseCurrency = d.config.baseCurrency || 'BRL';
  ...
  async function saveUserData(email,data){await init();const key=_k(email);const sanitized=sanitize(data);_dataCache.set(key,sanitized);await EphyraDB.put('userdata',{email:key,data:sanitized,updatedAt:new Date().toISOString()});try{await _syncSlices(key,sanitized);}catch(e){console.warn('[syncSlices]',e);}}
