const EphyraOnboarding = (() => {
  ...
  const STEPS = [
    ...
    },
    {
      ...
      description: 'Cada boa prática financeira gera pontos de experiência (XP). Suba de nível registrando transações, poupando em metas, acessando o app diariamente ou desbloqueando conquistas. Há mais de uma dezena de troféus, incluindo uma conquista secreta!',
    ...
    },
    {
      id: 'ready',
      title: 'Tudo pronto!',
      description: 'Agora você já conhece as principais ferramentas do Ephyra Finance. Seus dados são armazenados localmente no IndexedDB deste navegador e não são enviados para um servidor pela aplicação. Comece registrando suas movimentações diárias e mantenha também um backup dos dados importantes.',
  ...
  function renderCard() {
    ...
    const imagePath = `./images/tutorial/${step.image}`;

    return `
      <div class="onboarding-card anim-scale" role="document" tabindex="-1">
        ...
        <div class="onboarding-image-side">
          <div class="onboarding-image-container">
            <img class="onboarding-real-image" src="${imagePath}" alt="${step.title}" onload="this.classList.add('is-loaded');" onerror="this.classList.remove('is-loaded'); this.style.display='none';">
            <div class="onboarding-placeholder">
              <span class="onboarding-placeholder-icon">${step.iconChar}</span>
              <span class="onboarding-placeholder-tag">Imagem indisponível</span>
            </div>
          </div>
        </div>
      </div>
  ...
  function ensureScreen() {
    ...
    if (!screen) {
      ...
      screen.setAttribute('aria-label','Tutorial do Ephyra Finance');
  ...
  function show() {
    ...
    requestAnimationFrame(()=>screen.querySelector('.onboarding-card')?.focus());
  ...
  function handleKeydown(e) {
    ...
    if(e.key==='Tab'){
      ...
      const items=[...(screen?.querySelectorAll('button:not([disabled]),[tabindex]:not([tabindex="-1"])')||[])].filter(item=>item.offsetParent!==null);
  ...
  function hide() {
    ...
    if(!document.querySelector('.mo.a')&&!document.getElementById('sidebar')?.classList.contains('o'))document.body.classList.remove('ui-locked');
    previousFocus?.focus?.();
  }
